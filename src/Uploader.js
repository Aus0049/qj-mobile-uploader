...
class Uploader extends React.Component{
    ...
    handleInputChange (event) {
        ...
        const imgPass = {typeError: false, sizeError: false};
        ...
        Promise.all(promiseArray)
            .then(()=>{
                ...
                firstUpload.value.map((item)=>{
                     ...
                     */
                    if(canCompress){
                        this.compress(item, this.processData);
                    } else {
                        this.processFormDataForIos(item);
                    }
                });
            });
    }
    ...
    *uploadGenerator (uploadQueue) {
    ...
    getImagesListDOM () {
        ...
        const result = imgArray.map((item)=>{
            // 正在上传的图片
            if(item.status === 1){
        ...
        const onPress = ()=>{this.refs.input.click()};
